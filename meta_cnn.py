...
pd.DataFrame(stacked_train).to_csv("cnn_stacked_train.csv", index=True)
pd.DataFrame(stacked_test).to_csv("cnn_stacked_test.csv", index=True)

# Get the correct input shape for the meta-model
meta_input_shape = (stacked_train.shape[1], 1)  # (features, 1)

# Reshape stacked_train to match Conv1D expected input
stacked_train = stacked_train.reshape(stacked_train.shape[0], stacked_train.shape[1], 1)
stacked_test = stacked_test.reshape(stacked_test.shape[0], stacked_test.shape[1], 1)

# Train meta-model
meta_model = create_meta_model(meta_input_shape)
meta_model.fit(stacked_train, y_train, epochs=10, batch_size=32, validation_split=0.2, verbose=0)

# Save the meta-model
meta_model.save(os.path.join(base_dir, 'cnn_meta_model.h5'))
...
# Save all metrics
all_metrics_train_df.to_csv(os.path.join(base_dir, 'rerun_CNN_stacked_model_train_metrics.csv'))
all_metrics_test_df.to_csv(os.path.join(base_dir, 'rerun_CNN_stacked_model_test_metrics.csv'))
...
def run_ad(meta_model, stacked_test, y_test, name, z=0.5):
    ...
    df_metrics.to_csv(f"cnn_AD_metrics_{name}_{z}.csv", index=False)
    ...
    # Save the plot
    plt.tight_layout()
    plt.savefig(f"AD_{name}_{z}_cnn_ampk_AD.svg", bbox_inches='tight')
    plt.close()
...
def y_randomization(stacked_train, stacked_test, y_train, y_test, all_metrics_train_df, all_metrics_test_df):
    ...
    plt.tight_layout()
    plt.savefig("cnn_Y-randomization_ampk.svg", bbox_inches='tight')
    plt.close()
...
def plot_roc_and_confusion_matrices(y_train, y_prob_stacked_train, y_test, y_prob_stacked_test):
    ...
    plt.savefig("cnn_ROC_Curve_Train_Test.svg", bbox_inches='tight')
    plt.close()
    ...
    plt.savefig("cnn_Confusion_Matrix_Train_ampk.svg", bbox_inches='tight')
    plt.close()

    disp_test.plot(cmap='Oranges')
    plt.title('Test Confusion Matrix', fontsize=16, fontweight='bold', color=highlight_color)
    plt.savefig("cnn_Confusion_Matrix_Test_ampk.svg", bbox_inches='tight')
    plt.close()
